...
def search_for_layer(flops_op_dict, arch_def, flops_minimum, flops_maximum):
    sta_num = [1] * len(arch_def)
    # Order regulates which stage in limits gets the limit
    order = list(range(len(arch_def))) * 12
    # Limits regulate the limit of blocks per stage
    # limits = [10] * len(arch_def)
    size_factor = 224 // 32
    ...
    #     cur_ptr = 0
    #     while base_max_flops < flops_minimum and cur_ptr <= len(arch_def) - 1:
    ...
    #         break
     ...
     # [0] for stem
    for idx, stage in enumerate(arch_def):
        for block_idx, block in enumerate(stage):
            print(f'idx: {idx} block_idx: {block_idx} block: {block}')
            if block_idx != 0:
                sta_num[idx] += 1
    ...
    print('sta_num', sta_num, f'len of sta_num: {sta_num}')
