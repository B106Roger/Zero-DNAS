...
def build_trasition_prob_matrix(file_handler, step):
    ...
    for k in ['flops', 'resolution', 'n_bottlenecks', 'gamma']:
        prob_map[k] = {}
...
class ArchSampler():
    def __init__(self, arch_to_flops_map_file_path, discretize_step, flops_estimator, prioritized_board, acc_predictor=None):
        ...
        self.prioritized_board = prioritized_board
    ...
    def sample_archs_according_to_flops(self, target_flops, ignore_stages, block_to_choice_map, n_samples=3, max_trials=100, return_flops=True, return_trials=False):
        archs = []
        #for _ in range(n_samples):
        while len(archs) < n_samples:
            for _trial in range(max_trials+1):
                ...
                for k in ['n_bottlenecks', 'gamma']:
                    arch[k] = []
                    for idx in sorted(list(self.prob_map[k].keys())):
                        # print(target_flops, self.prob_map[k][idx])
                        arch[k].append(sample_helper(target_flops, self.prob_map[k][idx]))

                # 
                if self.flops_estimator:
                    parsed_arch = {'arch': self.prioritized_board.parse_attentive_candidate(
                        arch, 
                        ignore_stages=ignore_stages, 
                        block_to_choice_map=block_to_choice_map
                    )}
                    flops = self.flops_estimator.get_flops(parsed_arch['arch'])
                    if return_flops:
                        parsed_arch['flops'] = flops
                    if round_flops(flops, self.discretize_step) == target_flops:
                        break
                else:
                    raise NotImplementedError
            #accepte the sample anyway
            archs.append(parsed_arch)
        return archs
