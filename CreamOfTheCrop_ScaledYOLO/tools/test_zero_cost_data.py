...
def main():
    ...
    for iter_idx, (uimgs, targets, paths, _) in enumerate(dataloader_thetas):
        if iter_idx == SAMPLE_DATA_POINT: break
        ...
        # drop_mask = torch.nn.functional.dropout(drop_mask, p=0.5, training=True)  # uint8 to float32, 0-255 to 0.0-1.0
        ...
        if iter_idx < SAMPLE_DATA_POINT: 
            ...
            my_masks = [1,2,3]
            for i in range(3):
                m_name = str(Path(logdir) / (f'masks_batch{iter_idx}_{masks[i].shape[2]}.jpg' ))  # filename
                my_masks[i] = masks[i].cpu().float().numpy().transpose(0,2,3,1).repeat(3, axis=-1)
                top_img = np.concatenate(my_masks[i][0:2], axis=0)
            ...
            fore_list = np.array([score[iter_idx][0] for score in score_list]).reshape((-1,))
            back_list = np.array([score[iter_idx][1] for score in score_list]).reshape((-1,))
            sen_list  = np.array([score[iter_idx].sum() for score in score_list]).reshape((-1,))
            fore_list_sort = fore_list.argsort().argsort()
            back_list_sort = back_list.argsort().argsort()
            sen_list_sort  = sen_list.argsort().argsort()
            ...
            fig, ax = plt.subplots()
            p1=ax.bar(x1, fore_list,  color='b',  width=0.25, align='edge')  # 第一組數據靠左邊緣對齊 0.4
            p2=ax.bar(x2, back_list,  color='r',  width=0.25, align='edge')  # 第二組數據置中對齊
            p3=ax.bar(x3, sen_list,   color='g',  width=0.25)                 # 第二組數據置中對齊
            ax.bar_label(p1, labels=[str(item) for item in fore_list_sort])
            ax.bar_label(p2, labels=[str(item) for item in back_list_sort])
            ax.bar_label(p3, labels=[str(item) for item in sen_list_sort])

            plt.xticks(x1, x_str)
            plt.yticks(y1, y_str)
            ax.set_xlabel('Arch')
            ax.set_ylabel('Sensitivity (1e-6)')
            ax.legend(['foreground', 'background', 'sensitivity'])
            # fig.set_size_inches(15.5, 13.5)
            fig.tight_layout()
            fig.savefig(str(Path(logdir) /f'score_{iter_idx}.jpg'))
    ...
    for fn_idx, (kind, init_fn) in enumerate(init_fn_list):
        ...
        ######################################
        if True: 
            # result = plot_images(images=imgs,       targets=targets, paths=paths, fname=str(Path(output_dir) / f'imgs_batch{iter_idx}.jpg'))
            # result = plot_images(images=imgs_aug,   targets=targets, paths=paths, fname=str(Path(output_dir) / f'augs_batch{iter_idx}.jpg'))
            logdir = output_dir 
            # my_masks = [1,2,3]
            # for i in range(3):
            #     m_name = str(Path(logdir) / (f'masks_batch{iter_idx}_{masks[i].shape[2]}.jpg' ))  # filename
            ...
            fore_list = np.array([score[iter_idx+fn_idx][0] for score in score_list]).reshape((-1,))
            back_list = np.array([score[iter_idx+fn_idx][1] for score in score_list]).reshape((-1,))
            sen_list  = np.array([score[iter_idx+fn_idx].sum() for score in score_list]).reshape((-1,))
            fore_list_sort = fore_list.argsort().argsort() 
            back_list_sort = back_list.argsort().argsort()
            sen_list_sort  = sen_list.argsort().argsort()
            print(fore_list)
            ...
            ax.legend(['foreground', 'background', 'sensitivity'])
            fig.tight_layout()
            fig.savefig(str(Path(logdir) /f'score_{iter_idx}_{kind}_{fn_idx}.jpg'))
