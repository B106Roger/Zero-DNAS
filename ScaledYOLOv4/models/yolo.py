...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # if m in [BottleneckCSP, BottleneckCSP2]:
        #     print(f'[Roger] #{csp_bottle_count} bottle neck. gamma: {csp_gammas[csp_bottle_count]}')
        #     m_ = nn.Sequential(*[m(*args, e=csp_gammas[csp_bottle_count]) for _ in range(n)]) if n > 1 else m(*args, e=csp_gammas[csp_bottle_count])  # module
        #     csp_bottle_count += 1
        # else:
