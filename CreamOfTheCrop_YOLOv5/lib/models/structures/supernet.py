...
class SuperNet(nn.Module):
    ...
    def __init__(
        ...
        self.hetero_choices = [9, 6, 3, 0]
        # self.choices = calculate_choices(choices)
        self.choices = 12
        self.block_to_choice_map = []
        self.ignore_stages = [0, 2, 4, 6, 7, 9, 10, 12, 13, 15, 16, 18, 19, 21]
        self.temperature = 3
        ...
        s = 256  # 2x min stride
        forward_once, _, _ = self.forward(torch.zeros(1, 3, s, s), self.random_cand, first_run=True)
        ...
        for layer, layer_arch in zip(self.blocks, architecture):    
            for blocks, arch in zip(layer, layer_arch):
                ...
                if chosen_block.block_args.get('last') != None:
                    x = [y[j] for j in [16, 19, 22]]
                    print([x == None for x in y])
...
def gen_supernet(flops_minimum=0, flops_maximum=600, **kwargs):
    ...
    choices = {'n_bottlenecks': [9, 6, 3, 0], 'gamma': [0.25, 0.5, 0.75]}
    ...
    arch_def = [
        ...
        # stage 7
        ['sppcsp_r1_k1_s1_cin1024_cout1024'],
        # stage 8
        ['bottlecsp_r1_k1_s1_num8_gamma0.5_cin1024_cout1024'], # BottleneckCSP [1024]
        # stage 9
        ['cn_r1_k1_s1_cin1024_cout512', 'up_r1_mnearest_sf2_c512'],
        # stage 10
        ['concat_r1_c1024_f5'],
        # stage 11
        # ['bottlecsp2_r1_k1_s1_num2_gamma0.5_cin512_cout256'], # should be csp2
        ['bottlecsp_r1_k1_s1_num2_gamma0.5_cin1024_cout512'], # for C3 yolov5
        # stage 12
        ['cn_r1_k1_s1_cin512_cout256', 'up_r1_mnearest_sf2_c256'],
        # stage 13
        ['concat_r1_c512_f3'],
        # stage 14
        # ['bottlecsp2_r1_k1_s1_num2_gamma0.5_cin256_cout128'],
        ['bottlecsp_r1_k1_s1_num2_gamma0.5_cin512_cout256'], # for C3 yolov5
        # stage 15
        ['cn_r1_k3_s2_cin256_cout256'],
        # stage 16
        ['concat_r1_c512_f14'],
        # stage 17
        # ['bottlecsp2_r1_k1_s1_num2_gamma0.5_cin512_cout256'], #should be csp2
        ['bottlecsp_r1_k1_s1_num2_gamma0.5_cin512_cout512'], # for C3 yolov5
        # stage 18
        ['cn_r1_k3_s2_cin512_cout512'],
        # stage 19
        ['concat_r1_c1024_f10'],
        # stage 20
        # ['bottlecsp2_r1_k1_s1_num2_gamma0.5_cin1024_cout512'], #should be csp2
        ['bottlecsp_r1_k1_s1_num2_gamma0.5_cin1024_cout1024_last'], # for C3 yolov5
    ]
